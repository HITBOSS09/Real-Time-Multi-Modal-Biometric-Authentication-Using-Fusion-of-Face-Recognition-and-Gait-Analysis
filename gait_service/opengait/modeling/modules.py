...
# ``modules.py`` is located under ``opengait/modeling``.  When the project
# root is added to ``sys.path`` (see the various scripts in domain_adapt and
# opengait/main.py) the top-level package ``opengait`` is visible.  Previous
# code incorrectly attempted to import a bare ``utils`` module, which only
# worked when ``opengait`` was itself placed on ``sys.path`` directly.  This
# change uses a relative import so that the dependency is unambiguous.
